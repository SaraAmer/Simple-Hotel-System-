...
class FloorsController extends Controller
{
    ...
    public function data()
    {
        if (Auth::user()->hasRole('admin')) {
            ...
            ->addColumn('Manger_name', function ($floors) {
                return $floors->user[0]->name;
            })

            ->addColumn('action', function ($floors) {
                $delUrl = "/floors/" . $floors->number;
                return '<a href="/floors/'.$floors->number.'/edit" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i> Edit</a>
