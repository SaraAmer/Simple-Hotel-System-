...
<div class="card">
    ...
    <tr>
        ...
        <td>{{ $floor->number }}</td>
        {{
            $floor->manager ?  $floor->manager->name : 'floor not found'
          }}
        <td>

            <a href="{{route('floors.edit',['floors' => $floors['number']])}}" class="btn btn-secondary"
                style="margin-bottom: 20px;">Edit</a>
            <form style="display:inline" method="POST"
                action="{{route('floors.destroy',['floors' => $floors['number']])}}">
                @csrf
                @method('DELETE')
